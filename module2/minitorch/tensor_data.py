...
def count(position, shape, out_index):
    ...
    """
    # TODO: Implement for Task 2.1.
    # print(f'Array length: {len(out_index)}')
    # print(f'Shape: {shape}')
    ...
    for i in range(0, len(strides)):
        if position == 0:
            out_index[i] = 0
            continue
        out_index[i] = int(position // strides[i])
        position -= int(out_index[i] * strides[i])
    # print(f'Position: {position} Out Index: {out_index}')
    # raise NotImplementedError('Need to implement for Task 2.1')
...
def shape_broadcast(shape1, shape2):
    ...
    if len(shape1) < len(shape2):
        difference = len(shape2) - len(shape1)
    ...
    for i in range(len(shape1)):
        if shape1[i] != shape2[i]:
            if shape1[i] != 1 and shape2[i] != 1:
                raise IndexingError(f'These two shapes, shape1: {tuple(shape1)}, shape2: {tuple(shape2)} cannot be broadcasted')
            else:
                if shape1[i] == 1:
...
class TensorData:
    ...
    def indices(self):
        ...
        for i in range(self.size):
            ...
            # if time == 0:
            #    print(f'Repetition: {time} Position: {i} Shape: {self.shape} Multi Dimensional Index: {out_index}')
            # print(f'Out Index: {}')
    ...
    def permute(self, *order):
        ...
        # TODO: Implement for Task 2.1.
        permuted_shape = [self._shape[shape] for shape in order]
        permuted_strides = [self._strides[shape] for shape in order]
        x = TensorData(self._storage, tuple(permuted_shape), tuple(permuted_strides))
        return x
        # raise NotImplementedError('Need to implement for Task 2.1')
