    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        for i in range(0, len(inputs)):
            if not is_constant(inputs[i]):
                variables.append(VariableWithDeriv(inputs[i], derivatives[i]))
        return variables
...
def backpropagate(final_variable_with_deriv):
    ...
    while not q.empty():
        var = q.get()
        if is_leaf(var.variable):
            var.variable._add_deriv(var.deriv)
            continue
        variables = var.variable.history.backprop_step(var.deriv)
