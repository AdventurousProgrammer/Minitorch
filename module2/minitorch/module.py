...
class Module:
    ...
    # before going to next node add '.' to previous path
    def collect_parameters(self, path, index=0):
        global all_parameters
        for parameter, value in self._parameters.items():
            all_parameters[path + str(parameter)] = value
        for name, module in self._modules.items():
            all_parameters = module.collect_parameters(path + name + '.')  # not sure what to do about the x here

        return all_parameters  # hello

    def named_parameters(self):
        path = ''
        global all_parameters
        all_parameters = dict()
        self.collect_parameters(path)
        # for parameters in all_parameters.keys():
